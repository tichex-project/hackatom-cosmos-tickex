...
func NewGaiaApp(logger log.Logger, db dbm.DB, traceStore io.Writer, loadLatest bool,
	...
	app.issueKeeper = issue.NewBaseKeeper(app.issueKeeper, issueSubspace, issue.DefaultCodespace)
	...
	app.mm.SetOrderInitGenesis(genaccounts.ModuleName, distr.ModuleName,
		...
		gov.ModuleName, mint.ModuleName, crisis.ModuleName, genutil.ModuleName, issue.ModuleName)
	...
	app.MountStores(app.keyMain, app.keyAccount, app.keyStaking, app.keyMint,
		...
		app.keyParams, app.tkeyParams, app.tkeyStaking, app.tkeyDistr, app.keyIssue)
